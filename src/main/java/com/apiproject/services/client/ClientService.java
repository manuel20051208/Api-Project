package com.apiproject.services.client;
...
public class ClientService {
    ...
    @Transactional(readOnly = true)
    @Cacheable(value = CacheConstants.CLIENT_RESPONSE, key = "#id", sync = true)
    ...
    @Transactional(readOnly = true)
    @Cacheable(value = CacheConstants.CLIENT_PROFILE_PHOTO, key = "#clientId", sync = true)
    ...
    @Transactional
    @Caching(
            put = @CachePut(value = CacheConstants.CLIENT_RESPONSE, key = "#clientId"),
            evict = @CacheEvict(value = CacheConstants.CLIENT_PROFILE_PHOTO, key = "#clientId")
    )
    ...
    @Transactional
    @CachePut(value = CacheConstants.CLIENT_RESPONSE, key = "#id")
    ...
    @Transactional
    @Caching(evict = {
            @CacheEvict(value = CacheConstants.PAYMENT_CARDS, key = "#authenticatedClientId"),
            @CacheEvict(value = CacheConstants.CLIENT_RESPONSE, key = "#authenticatedClientId")
    })
    ...
    @Transactional(readOnly = true)
    @Cacheable(value = CacheConstants.PAYMENT_CARDS, key = "#authenticatedClientId", sync = true)
    ...
    public PaymentCardResponseDTO updatePaymentCardStatus(Long cardId, boolean active, Long authenticatedClientId) {
        ...
        int updatedRows = paymentCardRepository.updateActiveByIdAndUserClientId(cardId, authenticatedClientId, active);
        if (updatedRows == 0) {
            throw new ResponseStatusException(BAD_REQUEST, "Tarjeta no encontrada o sin permiso: " + cardId);
        }
