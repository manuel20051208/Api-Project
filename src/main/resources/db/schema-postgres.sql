-- PostgreSQL schema for ApiProject.
-- Run this on a clean database, or review before running on a database with data.

BEGIN;

-- Limpieza previa de vistas y tablas si ya existen
DO $$
DECLARE
db_object RECORD;
BEGIN
...
WHERE n.nspname = 'public'
  AND c.relname IN (
      'view_of_dashboard', 'clients_summary', 'view_of_sales', 'report_view_dashboard',
      'cupons', 'product_cupons_applied', 'cupons_used_by_clients',
      'secondhand_product', 'secondhand_product_images',
      'secondhand_cupons', 'secondhand_product_cupons_applied', 'sh_cupons_used_by_clients',
      'sh_sales', 'sh_sales_item',
      'services_offered', 'services_cupon',
      'product_cupon_to_a_client', 'sh_product_cupo_to_a_client', 'service_cupon_to_a_client'
  )
    LOOP
        IF db_object.relkind = 'v' THEN
            EXECUTE FORMAT('DROP VIEW IF EXISTS %I CASCADE', db_object.relname);
ELSIF db_object.relkind = 'm' THEN
            EXECUTE FORMAT('DROP MATERIALIZED VIEW IF EXISTS %I CASCADE', db_object.relname);
        ELSIF db_object.relkind IN ('r', 'p') THEN
            EXECUTE FORMAT('DROP TABLE IF EXISTS %I CASCADE', db_object.relname);
END IF;
END LOOP;
END $$;
...
-- Índice recomendado para que la paginación por cliente + fecha sea rápida
-- (ya tienes uno muy similar: idx_sale_items_client_date, así que probablemente
-- no necesites crear otro; esta vista lo aprovecha directamente)
...
CREATE TABLE IF NOT EXISTS sh_cupons_used_by_clients (
    ...
    REFERENCES clients (id)
    ON DELETE CASCADE,
    -- FK corregida respecto al DBML: apunta a sh_sales, no a sales
    CONSTRAINT fk_shcubc_sh_sales
    FOREIGN KEY (sale_id)
    REFERENCES sh_sales (id)
    ON DELETE SET NULL,
    -- FK corregida respecto al DBML: apunta a secondhand_cupons, no a cupons
...
-- ========================================================
-- 6.1 ASIGNACIÓN DE CUPONES A CLIENTES
-- ========================================================
